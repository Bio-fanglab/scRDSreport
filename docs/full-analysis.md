...
| `differential` | 按 cell type/cluster 聚合的 pseudobulk、比较表、效应大小 | 正式推断要求明确的 sample/group 且每组至少两个独立生物学样本；否则默认输出无 P/FDR 的描述性 one-vs-rest 排序和效应量 |
| `enrichment` | GO、KEGG、GSEA/GSVA 及其表图 | 有正式排序时运行对应方法；只有探索性排序时可输出无显著性结论的基因集效应摘要 |
| `pseudotime` | 轨迹几何、候选起点，以及有显式 root 时的 pseudotime、动态基因和富集 | 需要足够细胞和可用降维；定向结论必须由用户提供 `trajectory_root` |
| `communication` | CellChat 网络、强度、配体受体/通路、发送者-接收者 | 推断需要可信细胞注释和匹配物种数据库；条件不足时只导出分组上下文/诊断，不宣称不存在通讯 |
...
| `cnv` | inferCNV 输入、参考、信号和染色体摘要/热图 | 推断必须有用户指定的正常参考组和经确认匹配 genome assembly 的坐标资源；否则只导出 readiness，不产生 CNV 信号 |
...
- 没有 CellChat 条件或结果为空：只输出分组上下文和计算诊断，不能据此声称生物学上没有细胞通讯；
- 没有正常参考组：`cnv` 显示 `cnv_reference_required`，可输出 readiness，但不生成 CNV 信号；
- 某个可选依赖未安装：只跳过对应模块；
...
- 任一组少于两个独立 sample：只输出组内汇总后的 pooled CPM、log2 fold change 等描述性结果，不生成正式 P 值；pooled CPM 不是样本级 CPM 的算术平均；
- sample/group 含义不明确：不做重复水平推断；若存在可用注释或 cluster，默认生成有界的 one-vs-rest 描述性排序和效应量，否则显示 `needs_input`；
- 用户显式选择 `wilcox`：可以给出 cell-level 探索结果，但细胞不是生物学重复，不能把其 P 值解释成组间生物学推断。

同样地，样本细胞组成图默认是描述性图，不等于有重复的组成差异检验。

默认最多生成 20 个 annotation/cluster one-vs-rest 结果和 6 个样本层比较，防止类别数较多时结果文件平方增长。描述性差异表的 `analysis_type`、说明和图标题都会明确标记“无显著性检验”；富集章节若只消费这类排序，也只给出基因集成员的平均/中位效应、方向、overlap 和 leading genes，不生成或暗示 P 值/FDR。
...
cfg <- report_config(
  ...
  module_options = list(
    cnv = list(object_genome_assembly = "GRCh38/hg38")
  )
)
running("tumor.rds", "tumor_result", species = "human", config = cfg)
```

还需要与对象 feature ID 和 genome assembly 匹配的 TxDb 或 GTF。内置 TxDb 不会仅凭 `species` 直接用于推断：用户必须声明与注册资源匹配的 `object_genome_assembly`，或核验后设置 `genome_assembly_confirmed = TRUE`；另一种做法是显式提供本对象对应的 `gene_order`、`gtf` 或 `txdb`。未提供 reference 时状态为 `cnv_reference_required`；reference 不在注释中、assembly 未确认、基因坐标无法匹配或 `infercnv` 未安装时，章节会保留 readiness 与具体原因。CNV 信号是表达推断结果，不应直接等同于 DNA 层面的拷贝数验证。
...
`report.html` 内置页面 CSS、JavaScript、图形和 DT 组件，可离线打开。左侧目录默认展开；文件索引的说明列更宽、每行固定高度并可独立滚动。可下载文件是否也内置由 `embed_downloads` 决定：
